...
def match_candidate_to_jobs_ai(candidate_data, jobs_data, limit=10):
    """
    Use OpenAI to match a candidate to potential jobs
    """
    try:
        # For demo purposes, return sample matches
        matches = [
        ...
        logger.error(f"Error matching candidate to jobs with OpenAI: {str(e)}")
        return []
...
def match_candidate_to_jobs():
    ...
    try:
        ...
        candidate_data = {
            ...
            'jobPreferences': data.get('jobPreferences', {})
        }
        
        limit = data.get('limit', 10)
        
        # Get matches using OpenAI
        matches = match_candidate_to_jobs_ai(candidate_data, [], limit)
        
        logger.info(f"Successfully matched candidate {candidate_data['candidateId']} to {len(matches)} jobs")
